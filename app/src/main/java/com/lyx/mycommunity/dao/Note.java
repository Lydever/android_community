...
public class Note implements NoteDataBaseHelper.TableCreateInterface {
    ...
    public static String pre = "pre";  // 设置微记帖子的优先级
...
//        return cursor;
    ...
    // 搜索查询数据 以HashMap<String, Object>键值对的形式获取一条信息
    public static HashMap<String, Object> getNote(NoteDataBaseHelper dbHelper, int _id) {
        ...
        cursor.moveToFirst();
        byte[]  imgarr = cursor.getBlob(cursor.getColumnIndex(Note.picture));
        Bitmap photo = BitmapFactory.decodeByteArray(imgarr,0,imgarr.length);
        ...
        NoteMap.put(Note.picture, photo);

        /*BitmapFactory.Options opts = new BitmapFactory.Options();
        BitmapFactory.decodeByteArray(cursor.getBlob(cursor.getColumnIndex(Note.picture)), 0, cursor.getBlob(cursor.getColumnIndex(Note.picture)).length, opts);
        */
        return NoteMap;
    }
